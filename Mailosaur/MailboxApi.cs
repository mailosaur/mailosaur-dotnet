...
namespace Mailosaur
{
	public class MailboxApi
	{
		...
		private readonly string m_MailboxId;
		...
		static MailboxApi()
		{
			...
			s_UserAgent = "Mailosaur .NET client " + typeof(MailboxApi).Assembly.GetName().Version;

		}

		public MailboxApi(string mailboxId, string apiKey)
		{
		...
		{
			return new Uri(string.Format("{0}{1}?{2}", BaseUrl, path, BuildQueryString(queryParams)));
		}
		...
		private Stream GetResponseStream(string method, string path, NameValueCollection queryParams = null)
		{
			...
			String encoded = Convert.ToBase64String(Encoding.GetEncoding("ISO-8859-1").GetBytes(m_ApiKey + ":"));
			request.Headers.Add("Authorization", "Basic " + encoded);
			request.Headers["UserAgent"] = s_UserAgent;
		...
		private Email[] GetEmails(NameValueCollection searchCriteria)
		{
			try
			{
				...
				return JsonConvert.DeserializeObject<Email[]>(GetResponse("GET", BuildUrlPath(true, "mailboxes", m_MailboxId, "emails"), queryParams));
			}
			catch (Exception e)
			{
		...
		public Email GetEmail(string emailId)
		{
			try
			{
				return JsonConvert.DeserializeObject<Email>(GetResponse("GET", BuildUrlPath(true, "emails", emailId)));
			}
			catch (Exception e)
			{
				throw new MailosaurException("Unable to parse API response", e);
		...
		public void DeleteAllEmail()
		{
			try
			{
				...
				GetResponse("POST", BuildUrlPath(true, "mailboxes", m_MailboxId, "empty"), queryParams);
			}
			catch (Exception e)
			{
				throw new MailosaurException("Unable to parse API response", e);
		...
		public void DeleteEmail(string emailId)
		{
			try
			{
				GetResponse("POST", BuildUrlPath(true, "emails", emailId, "delete"));
			}
			catch (Exception e)
			{
				throw new MailosaurException("Unable to parse API response", e);
		...
		{
			return GetResponseStream("GET", BuildUrlPath(false, "attachments", attachmentId));
		}
		...
		public string GenerateEmailAddress()
		{
			...
			return string.Format("{0}.{1}@" + SmtpHost, guid, m_MailboxId); 
		}
	}
}